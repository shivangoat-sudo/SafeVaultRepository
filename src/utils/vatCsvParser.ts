...
/**
 * Parse a bank CSV into structural raw transactions only.
 * No VAT classification or financial calculation occurs here.
 */
export function parseCsvToRawTransactions(csvContent: string): RawTransaction[] {
  ...
  if (parseResult.errors.length > 0) {
    throw new Error(`CSV kon niet veilig worden gelezen: ${parseResult.errors[0].message}`);
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const isIncome = rawAfBij === 'bij' || rawAfBij === 'income' || rawAfBij === 'inkomsten' || rawAfBij === 'c' || rawAfBij === 'cr'
      ? true
      : rawAfBij === 'af' || rawAfBij === 'expense' || rawAfBij === 'uitgaven' || rawAfBij === 'd' || rawAfBij === 'dr'
        ? false
        : parsedAmount > 0;
