...
assert(gallTx?.btw?.percentage === 21, 'GALL & GALL zonder productwoord moet via merchant-context op 21% uitkomen.');
...
assert(nsTx?.btw?.percentage === 9, 'NS moet als 9% personenvervoer worden herkend.');
...
assert(mixedTx?.btw?.percentage === 21, 'Bij gelijktijdige 9%- en 21%-signalen moet de bankregel volgens projectbeleid op 21% uitkomen.');
...
assert(unknownTx?.btw?.percentage == null, 'Een unresolved transactie mag geen verzonnen btw-percentage bevatten.');
