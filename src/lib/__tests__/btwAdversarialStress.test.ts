...
const adversarialRows: RawTransaction[] = [
  income('a01', 'Verkoop zakelijke dienstverlening', 121), income('a02', 'Verkoop boek', 109), income('a03', 'Export goederen buiten EU', 1000),
  expense('a04', 'Laptop computer zakelijke aankoop', 121), expense('a05', 'Supermarkt voedingsmiddelen', 109), expense('a06', 'Hotel overnachting zakelijke reis 2026', 121),
  expense('a07', 'OpenAI LLC', 121), expense('a08', 'Adobe Systems Software', 121), expense('a09', 'Salaris medewerker', 2500), expense('a10', 'Aflossing lening bank', 500),
  expense('a11', 'Belastingdienst btw-aangifte', 300), expense('a12', 'Bankkosten rekening', 12.10), expense('a13', 'Café De Hoek'), expense('a14', 'Jumbo Zakelijk boodschappen'),
  expense('a15', 'Hotel De Zon all-in ontbijt'), expense('a16', 'Kliniek Tandheelkunde cosmetisch bleken'), expense('a17', 'Betaling leverancier 21%', 121), expense('a18', 'Betaling leverancier 9%', 109),
  expense('a19', 'Café De Hoek alcohol lunch', 121), expense('a20', 'Café De Hoek 9%', 109), expense('a21', 'Café De Hoek alcohol wijn', 121), expense('a22', 'Café De Hoek lunch', 109),
  expense('a23', 'Café De Hoek factuur 21%', 121), expense('a24', 'Café De Hoek factuur 9%', 109), expense('a25', 'Café De Hoek omschrijving met Café en éé́n', 121),
  expense('a26', 'Pathé bioscoopkaartje', 109), expense('a27', 'PostNL Pakketten', 121), expense('a28', 'Albert Heijn Zakelijk 9%', 109), expense('a29', 'Didi Talks NL marketing', 121), expense('a30', 'Advocatenkantoor Meijer', 121),
];
...
expectKnown('a02', 'domestic_output_9', '1b', 9); expectKnown('a03', 'non_eu_output_0', '3a', 0); expectKnown('a04', 'domestic_input_21', '5b', 21);
expectKnown('a05', 'domestic_input_9', '5b', 9); expectKnown('a06', 'domestic_input_21', '5b', 21); expectKnown('a07', 'non_eu_reverse_charge', '4a', 21);
expectKnown('a08', 'eu_reverse_charge', '4b', 21); expectKnown('a09', 'private_no_vat', 'geen', 0); expectKnown('a10', 'private_no_vat', 'geen', 0);
expectKnown('a11', 'private_no_vat', 'geen', 0); expectKnown('a12', 'private_no_vat', 'geen', 0); expectKnown('a17', 'domestic_input_21', '5b', 21);
expectKnown('a18', 'domestic_input_9', '5b', 9); expectKnown('a20', 'horeca_bua_9', '5b', 9); expectKnown('a22', 'horeca_bua_9', '5b', 9);
expectKnown('a23', 'horeca_bua_9', '5b', 9); expectKnown('a24', 'horeca_bua_9', '5b', 9); expectKnown('a26', 'domestic_input_9', '5b', 9); expectKnown('a27', 'domestic_input_21', '5b', 21);
expectKnown('a28', 'domestic_input_9', '5b', 9); expectKnown('a29', 'domestic_input_21', '5b', 21); expectKnown('a30', 'domestic_input_21', '5b', 21);
for (const id of ['a01', 'a13', 'a14', 'a15', 'a16', 'a19', 'a21']) { const tx = byId(id); assert.equal(tx.vat.status, 'unknown', `${id}: ambigue bankdata mag niet gokken`); assert.equal(tx.includedInTotals, false, `${id}: ambigue transactie mag totalen niet vervuilen`); }
...
for (const [label, csv] of parserCases) {
  ...
  try { parsed = parseCsvToRawTransactions(csv); } catch (error) { throw new Error(`${label} parser-case failed: ${error instanceof Error ? error.message : String(error)}`); }
