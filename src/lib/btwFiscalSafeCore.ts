...
export const BOEKHOUDER_PERCENTAGE_OPTIES = [
  {percentage:0 as const,label:'0%'},{percentage:9 as const,label:'9%'},{percentage:21 as const,label:'21%'}
];
...
export interface FiscalTransaction {
  id:string; date?:string; description?:string; type:'income'|'expense'; amount_incl_input:number;
  amount_excl:number|null; vat:{status:'known';rate:number;amount:number}|{status:'unknown';rate:null;amount:null};
...
function section(x:FiscalClassification):FiscalSection {
  if(x==='domestic_output_21')return'1a'; if(x==='domestic_output_9')return'1b'; if(x==='other_rate_output')return'1c'; if(x==='private_use_adjustment')return'1d';
  if(x==='zero_rated_output'||x==='exempt_output')return'1e'; if(x==='domestic_reverse_charge')return'2a'; if(x==='non_eu_output_0')return'3a'; if(x==='eu_output_0')return'3b';
  if(x==='non_eu_reverse_charge')return'4a'; if(x==='eu_reverse_charge')return'4b';
  if(x.startsWith('domestic_input_')||x.startsWith('non_deductible_input_')||x==='horeca_bua_9'||x==='zero_rated_input'||x==='exempt_input')return'5b';
  return'geen';
}
function rule(x:FiscalClassification):FiscalRule {
  ...
  const explanation:Record<FiscalSection,string>={
    ...
    '3a':'Uitvoer buiten de EU; 0% alleen indien aan de voorwaarden voor uitvoer is voldaan.',
    '3b':'Intracommunautaire levering/dienst; voorwaarden en ICP-verplichtingen moeten kloppen.',
    '4a':'Inkoop buiten de EU met verlegging.', '4b':'Inkoop uit de EU met verlegging.',
    '5a':'Totaal verschuldigde btw.', '5b':'Voorbelasting; aftrek alleen voor zover wettelijk toegestaan.', 'geen':'Geen financiële btw-post.'
  };
...
export function calculateFiscalVatReport(rows:RawTransaction[],overrides:Record<string,BoekhouderBeoordeling>={},adjustments:FiscalAdjustments={}):FiscalReport {
  ...
  let o21=0,o9=0,oo=0,pu=0,dr=0,er=0,nr=0,i21=0,i9=0,ri=0,nd=0;
  for(const t of transactions){const v=t.vat.status==='known'?t.vat.amount:0;const b=t.amount_excl??0;if(!t.includedInTotals||t.vat.status!=='known')continue;
    switch(t.section){
      case'1a':aangifte['1a'].grondslag=add(aangifte['1a'].grondslag,b);aangifte['1a'].btw=add(aangifte['1a'].btw,v);o21=add(o21,v);break;
      case'1b':aangifte['1b'].grondslag=add(aangifte['1b'].grondslag,b);aangifte['1b'].btw=add(aangifte['1b'].btw,v);o9=add(o9,v);break;
      case'1c':aangifte['1c'].grondslag=add(aangifte['1c'].grondslag,b);aangifte['1c'].btw=add(aangifte['1c'].btw,v);oo=add(oo,v);break;
      case'1d':aangifte['1d'].grondslag=add(aangifte['1d'].grondslag,b);aangifte['1d'].btw=add(aangifte['1d'].btw,v);pu=add(pu,v);break;
      case'1e':aangifte['1e'].grondslag=add(aangifte['1e'].grondslag,b);break;
      case'2a':aangifte['2a'].grondslag=add(aangifte['2a'].grondslag,b);aangifte['2a'].btw=add(aangifte['2a'].btw,v);dr=add(dr,v);if(t.deductible)ri=add(ri,v);break;
      case'3a':aangifte['3a'].grondslag=add(aangifte['3a'].grondslag,b);break;
      case'3b':aangifte['3b'].grondslag=add(aangifte['3b'].grondslag,b);break;
      case'4a':aangifte['4a'].grondslag=add(aangifte['4a'].grondslag,b);aangifte['4a'].btw=add(aangifte['4a'].btw,v);nr=add(nr,v);if(t.deductible)ri=add(ri,v);break;
      case'4b':aangifte['4b'].grondslag=add(aangifte['4b'].grondslag,b);aangifte['4b'].btw=add(aangifte['4b'].btw,v);er=add(er,v);if(t.deductible)ri=add(ri,v);break;
      case'5b':if(t.deductible&&t.classification==='domestic_input_21')i21=add(i21,v);if(t.deductible&&t.classification==='domestic_input_9')i9=add(i9,v);break;
    }
    if(t.type==='expense'&&!t.deductible)nd=add(nd,v);
  }
  const m1c=Number(adjustments.rubriek1c?.btw??0),m1d=Number(adjustments.rubriek1d?.btw??0);
  if(adjustments.rubriek1c)aangifte['1c']={grondslag:Number(adjustments.rubriek1c.grondslag??0),btw:m1c};
  if(adjustments.rubriek1d)aangifte['1d']={grondslag:Number(adjustments.rubriek1d.grondslag??0),btw:m1d};
  const out=add(add(add(add(add(o21,o9),oo),dr),add(er,nr)),add(m1c,m1d));
