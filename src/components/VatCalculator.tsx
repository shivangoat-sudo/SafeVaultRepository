...
const AANGIFTE_RUBRIEKEN = ["1a", "1b", "1e", "2a", "3a", "3b", "4a", "4b"] as const;
...
export function VatCalculator({ customerId }: { customerId: string }) {
  ...
  return <div className="space-y-4">
    ...
    {report&&<div className="space-y-4">
      ...
      <div className="card overflow-hidden"><div className="p-4 bg-ink-50"><h3 className="font-semibold">Aangiftecontrole</h3><p className="text-xs text-ink-500">De engine toont de grondslag en btw per relevante Nederlandse aangifterubriek. Niet-onderbouwde posten blijven buiten de financiële totalen.</p></div><div className="grid md:grid-cols-3 gap-3 p-4 text-xs">{AANGIFTE_RUBRIEKEN.map(k=><div key={k} className="rounded border p-3"><b>Rubriek {k}</b><div>Grondslag: {eur(report.aangifte[k].grondslag)}</div><div>BTW: {eur(report.aangifte[k].btw)}</div></div>)}<div className="rounded border p-3"><b>Rubriek 5b</b><div>Voorbelasting: {eur(report.aangifte['5b'])}</div></div></div></div>
