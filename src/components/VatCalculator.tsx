...
export function VatCalculator({ customerId }: { customerId: string }) {
  ...
  const matchingFiles = useMemo(() => files.filter(f =>
    f.category === "income_overview" && (!f.quarter || String(f.quarter).toUpperCase() === quarter) &&
    (!f.year || f.year === year) && (/\.(csv|txt)$/i.test(f.original_name))
  ), [files, quarter, year]);
  ...
  const runCalculation = async (file: File, nextOverrides = overrides) => {
    ...
    try {
      ...
      const next = calculateVatReport(rows, { percentageOverrides: nextOverrides });
      setRawTransactions(rows);
      setReport(next);
    } catch (e) {
      push("error", e instanceof Error ? e.message : "BTW-berekening mislukt.");
  ...
  const applyReview = (id: string, percentage: BtwPercentage) => {
    ...
    if (report) {
      const next = calculateVatReport(rawTransactions, { percentageOverrides: nextOverrides });
      setReport(next);
    }
  };
  ...
  return <div className="space-y-4">
    <div className="card p-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><h3 className="text-sm font-semibold">BTW-calculator</h3><p className="text-xs text-ink-400">Berekening loopt uitsluitend via de veilige BTW-safety boundary.</p></div>
        <div className="flex gap-3">
          ...
          <button onClick={selectAndCalculate} disabled={calculating || !matchingFiles.length} className="btn-primary">{calculating ? <Spinner /> : <Calculator className="h-4 w-4" />} Berekenen</button>
        </div>
      </div>
    </div>
