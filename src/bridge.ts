...
function toLegacyMetrics(report:FiscalReport):EngineResult['metrics']{
  ...
  return {totaal_incl_21:round2(income21.reduce((s,t)=>s+t.amount_incl_input,0)),totaal_excl_21:round2(income21.reduce((s,t)=>s+(t.amount_excl??0),0)),totaal_incl_9:round2(income9.reduce((s,t)=>s+t.amount_incl_input,0)),totaal_excl_9:round2(income9.reduce((s,t)=>s+(t.amount_excl??0),0)),totale_btw_21:report.overzicht.output.domestic21,totale_btw_9:report.overzicht.output.domestic9,niet_aftrekbare_btw:report.overzicht.nonDeductible,btw_eindsaldo:report.overzicht.netto,vatReport:report,totalRevenueIncl,totalRevenueExcl,outputVat21:report.overzicht.output.domestic21,outputVat9:report.overzicht.output.domestic9,totalOutputVat:report.overzicht.output.total,deductibleInputVat21:report.overzicht.input.domestic21,deductibleInputVat9:report.overzicht.input.domestic9,totalDeductibleInputVat:report.overzicht.input.total,nonDeductibleVatBUA:report.overzicht.nonDeductible,euVerlegdVat:report.overzicht.output.euReverse+report.overzicht.output.nonEuReverse,netVatResult:report.overzicht.netto,totalRowsProcessed:report.audit.input,rowsRequiringReview:report.audit.unresolved};
}
